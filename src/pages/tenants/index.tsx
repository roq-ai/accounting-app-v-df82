...
type ColumnType = ColumnDef<TenantInterface, unknown>;
...
export function TenantListPage(props: TenantListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getTenants({
        relations: ['user', 'bank_account.count', 'expense.count', 'invoice.count', 'tax.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TenantInterface>>(
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('invoice', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('tax', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'tax',
          header: 'Tax',
          accessorKey: 'tax',
          cell: ({ row: { original: record } }: any) => record?._count?.tax,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/tenants/view/${record.id}`} passHref legacyBehavior>
                <Button
