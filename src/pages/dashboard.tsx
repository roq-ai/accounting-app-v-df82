...
import { TaxListPage as ShowCaseList } from 'pages/taxes';

function DashboardPage() {
  const { session } = useSession();

  const expenseStat = useSWR<PaginatedInterface<ExpenseInterface>>(
    () => `/expenses`,
    () => getExpenses({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Expenses',
      result: expenseStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`taxes`} passHref legacyBehavior>
                  <Button
