...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['admin', 'accountant', 'viewer', 'tenant'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<TenantInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/tenants` : null),
    () => getTenants({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Tenants',
      path: isTenantUser ? `/tenants/view/${data?.[0]?.id}` : '/tenants',
      entity: 'tenant',
    ...
    { name: 'Expenses', path: '/expenses', entity: 'expense', service: AccessServiceEnum.PROJECT, icon: FiShoppingBag },
    { name: 'Invoices', path: '/invoices', entity: 'invoice', service: AccessServiceEnum.PROJECT, icon: FiFileText },
    { name: 'Taxes', path: '/taxes', entity: 'tax', service: AccessServiceEnum.PROJECT, icon: FiPercent },
